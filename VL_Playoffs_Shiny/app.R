...
ui <- fluidPage(
   ...
   # Application title
   titlePanel(h1("Varberg-Lindquist 2019 Playoff Predictions", align = 'center')),
      ...
      mainPanel(h3("How these predictions work:", align = 'center'),
                h6("I gather preseason point projections for each player from fantasyfootballanlytics.net, who claim
                   to have the most accurate projections. From the player projections, I project the points per game 
                   (proj_ppg) for each team
                   ...
                   season 30,000 times.", align = 'center'),
...
server <- function(input, output) {
   ...
   output$preds2019 <- DT::renderDataTable({
     df <- fread("VL_predictions_week_0.csv")
     df <- df %>% select(-ID)
     df <- datatable(df, options = list(dom = 't'))
     return(df)
   })
